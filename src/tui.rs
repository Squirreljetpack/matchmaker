...
use std::{io::{self, BufRead, Read, Write}, thread::sleep, time::Duration};
...
W: Write,
{
    ...
    pub area: Rect,
    pub sleep : u64
    // hacky state tracking below maybe
}

// given min height/desired percent
    ...
    pub fn new_with_writer(writer: W, config: TerminalConfig) -> Result<Self> {
        ...
        let area = if let Some(ref layout) = config.layout {
            ...
            if available_height < layout.min {
                ...
                options.viewport = Viewport::Fixed(area.clone());
